...
COLOR = {'Plasma membrane / surface': core.PALETTE_SINGLE[2],
         'Mitochondrial membrane': core.PALETTE_SINGLE[1],
         'Other membrane': core.PALETTE_SINGLE[4],
         'Non-membrane': '#bdbdbd'}
...
print(f'  spread across all fibers: {digestion["mc0_by_intensity"].min():.1f} to '
      f'{digestion["mc0_by_intensity"].max():.1f} % intensity-weighted')
fig, axes = plt.subplots(2, 2, figsize=(6.9, 5.4))
ax = axes[0, 0]
rng_a = np.random.default_rng(0)
groups_a = [digestion.loc[digestion['fiber_type'] == ft, 'mc0_by_intensity'].to_numpy()
...
ax.set_ylabel('Fully cleaved signal [%]', fontsize=FONTSIZE)
