...
for method in METHODS:
    ...
    x = (sub['organ'].map(xpos) + DODGE[method]
         + rng.uniform(-POINT_JITTER, POINT_JITTER, size=len(sub)))
    ax.scatter(x, sub['protein_groups'], s=POINT_SIZE, color=COLOR[method],
               alpha=0.85, edgecolor='black', linewidth=0.4, label=method,
               zorder=4)
...
fig, ax, cv_stats = core.plot_cv_violin(
    ...
    group_order=ORGAN_ORDER, palette=COLOR, as_percent=True, ylim=(0, 60),
