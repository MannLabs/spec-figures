"""Supplemental figure 3c — PCA of the FFPE organ runs, SPEC vs PAC (H032_E170)."""
...
# Six organs, six house hues. Method is carried by marker shape, not colour, so the
# coral here is an organ and not SPEC — the legend has to be read for that.
...
fig.tight_layout()
fig.savefig(os.path.join(OUTDIR, 'panel_c_pca_organs.pdf'), bbox_inches='tight')
fig.savefig(os.path.join(OUTDIR, 'panel_c_pca_organs.png'), dpi=300,
            bbox_inches='tight')

# ---------------------------------------------------------------------------
# How much of each PC is organ, and how much is method?
# ---------------------------------------------------------------------------
scores = res['transformed']
if 'organ' not in scores:          # plot_pca already merges sample_info in
...
scores.to_csv(os.path.join(OUTDIR, 'panel_c_pca_organs_sourcedata.csv'), index=False)
