...
PALETTE_DOUBLE = [
    '#FBA08D', '#C79EEA', '#9ECCEA', '#9EDCB8', '#FBE49C', '#F4ADCA',  # light
    '#FA7A61', '#B178E2', '#78B8E2', '#6BC88F', '#F5C842', '#E85D9A',  # dark
]
PALETTE_SINGLE = ['#FA7A61', '#B178E2', '#78B8E2', '#6BC88F', '#F5C842', '#E85D9A']
PALETTE_CV_CATEGORIES = ['#5cb85c', '#f0ad4e', '#d9534f']  # <10%, 10-20%, >=20%


# --- default atomic-panel figure size --------------------------------------
# The size of a single "atomic" panel (plot_pca / plot_volcano /
# plot_boxplot_with_points, when called with ``figsize=None``). Laid out on A4
# portrait (~6.7" usable row). Change it globally at runtime with
# ``core.set_default_panel_size(w, h)`` — no need to touch every call site.
...
def init_plotting():
    ...
    plt.rcParams['mathtext.fontset'] = 'dejavusans'  # math matches the body font
    ...
    plt.rcParams['pdf.fonttype'] = 42  # TrueType — text-as-text (editable) in PDF
    plt.rcParams['ps.fonttype'] = 42
    plt.rcParams['svg.fonttype'] = 'none'


# --- replicate-point sizing -------------------------------------------------
# Overlaid replicate dots are sized from the DRAWN BAR WIDTH, because that is
# what they sit on: a dot reads when it is a substantial fraction of its bar and
# looks wrong when it spills over the edges. Bar *count* is a poor proxy — a wide
# panel with 8 bars has wider bars than a narrow panel with 4 — which is why
# `_replicate_point_size` below is only the fallback for callers that cannot
# determine their bar width.
#
# Diameter, not area, is the thing that scales with the bar: matplotlib's `s` is
# an area in points², so the conversion squares and a 2x wider bar earns a 4x
# larger `s`. The clamp keeps both ends usable — below ~4.5 pt a dot with a white
# edge stops reading, and above ~9 pt four jittered replicates swamp the bar they
# describe.
...
def _filter_pivot_by_validity(
    ...
    *,
    ...
    if min_valid_fraction_in_any_group is not None:
        ...
        # Map the runs on the pivot's run axis to their group; runs missing
        # from sample_info are dropped from the per-group consideration.
...
def _resolve_highlights(
    ...
    if highlight_genes:
        ...
        for ent, gene in gene_lookup.items():
            # genes can be ;-separated (multi-gene protein groups); match any
