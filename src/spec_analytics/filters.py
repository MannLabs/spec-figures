...
def filter_outlier_runs(
    ...
    *,
    ...
    """
    # Imported lazily (call time) to avoid an import cycle: plotting imports
    # filters at module load, so filters must not import plotting at its top.
    ...
    # run -> cohort label (preserving insertion order).
    ...
    for i, cohort in enumerate(cohorts):
        ...
        if n_runs < min_runs_for_fit or vals.std() == 0:
            # Too few runs (or zero variance) to fit a 2-component GMM
            # reliably — keep everything with a warning.
            warnings.warn(
        ...
        try:
            ...
            # Equal-posterior point lies outside [min, max] — i.e. one
            # cluster dominates. Keep everything.
            boundary = float('nan')
            threshold = float('nan')
