"""Supplementary figure 5a — digestion completeness of the FFPE preparations."""
import os
import sys
import numpy as np
import pandas as pd
...
BATCH_ROWS = 500_000
FONTSIZE = 8
STEM = 'suppl5_a_digestion_completeness'
...
spec = ffpe.loc[ffpe['condition'] == 'Single-shot SPEC', 'mc0_by_intensity'].mean()
bulk = ffpe.loc[ffpe['condition'] == 'Bulk ISD+', 'mc0_by_intensity'].mean()
print(f'  SPEC minus bulk ISD+: {spec - bulk:+.1f} points')
table = organs.pivot_table(index='organ', columns='method',
                          values='mc0_by_intensity', aggfunc='mean').reindex(ORGAN_ORDER)
...
for i, condition in enumerate(ORDER):
    draw_points(ax, i, ffpe.loc[ffpe['condition'] == condition,
                                'mc0_by_intensity'].to_numpy(), COLOR[condition])
