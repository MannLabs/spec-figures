...
LEGEND_FONTSIZE = 9.5
AXES_H_IN = 2.50
# Panel width as saved: two panels per 8.4 in row, up from round 1's 3.90.
PANEL_W_IN = 4.20
...
def set_axes_size_inches(fig, axes, *, w_in=None, h_in=None):
    ...
    and kept, so only the data area changes size and every label keeps its
    clearance. `bbox_inches='tight'` crops without scaling, so the saved PDF
    keeps these inch values. `axes` may be a single axes or a list — a twinned
    pair has to be repositioned together or the two stop sharing a frame.
    """
...
def draw_grouped_dual(ax_left, ax_right, categories, series, *,
    ...
    **Only the left-axis series occupy bar slots**, so adding the second metric
    ...
    # twinx SHARES the x axis, so the ticks must be set once. Setting them on the
    # twin as well (or clearing them there) wipes the labels for both.
...
def read_peptides(path, *, runs=None):
    ...
    precursors' raw `Precursor.Quantity`, so charge states are summed and nothing
    else is. This is the definition `spec_analytics` uses for `peptide_id`
    ...
    columns = [c for c in ('Run', PEPTIDE_COLUMN, 'Precursor.Quantity',
    ...
    return (d.groupby(['Run', PEPTIDE_COLUMN], sort=False,
                      observed=True)['Precursor.Quantity']
...
def counts_and_cv(pep):
    ...
    values only — missing peptides are skipped, never imputed as zero, which is
