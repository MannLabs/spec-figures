...
FONTSIZE = 10          # the row scales ~0.82x to a 180 mm column; 10 pt prints 8.2
BAR_WIDTH_IN = 0.38    # the paper-wide drawn bar width
PANEL_W_IN = 2.1
AXES_H_IN = 2.05
...
for seg in SEGMENTS:
    ...
    for xi, (v, b, n) in enumerate(zip(vals, bottom, counts[seg].to_numpy())):
        # Only label a segment tall enough to hold the text; the rest are in the
        # printout and the source data.
        if v >= 7:
...
fig.tight_layout()
# Bars to the paper-wide drawn width, then the data area pinned so the panel and
# the strip beside it share a height.
fig.canvas.draw()
