"""Supplementary figure 2a — one SAX SPEC digest on two LC front ends (H032_E333)."""
import os
import sys
import numpy as np
import pandas as pd
...
OUTDIR = _cfg.output_dir_of('supplementary_figure2')
TRAP_COLUMN_COLOR = core.PALETTE_SINGLE[0]
ONLINE_COLOR = core.PALETTE_SINGLE[4]
CONDITIONS = [
...
fig.savefig(os.path.join(OUTDIR, 'suppl2_a_lc_comparison.pdf'), bbox_inches='tight')
fig.savefig(os.path.join(OUTDIR, 'suppl2_a_lc_comparison.png'), dpi=300,
            bbox_inches='tight')
...
pd.concat([pts[cols], pd.DataFrame(bars)[cols]], ignore_index=True).to_csv(
    os.path.join(OUTDIR, 'suppl2_a_lc_comparison_sourcedata.csv'), index=False)
